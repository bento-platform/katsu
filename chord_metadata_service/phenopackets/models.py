...
from chord_metadata_service.discovery.scopeable_model import TOP_LEVEL_MODEL_SCOPE_FILTERS, BaseScopeableModel
...
from chord_metadata_service.restapi.validators import JsonSchemaValidator, ontology_list_validator, ontology_validator
...
class MetaData(BaseTimeStamp, ToFTSReprMixin):
    ...
    created_by = models.CharField(
        max_length=200, blank=True, null=True, default=None, help_text=rec_help(d.META_DATA, "created_by")
    )
    submitted_by = models.CharField(
        max_length=200, blank=True, null=True, default=None, help_text=rec_help(d.META_DATA, "submitted_by")
    )
    ...
    updates = JSONField(
        ...
        null=True,
        validators=[
            JsonSchemaValidator(schema_ref=SchemaRefs.VALIDATION_PHENOPACKET_UPDATE_SCHEMA, formats=["date-time"])
        ],
        help_text=rec_help(d.META_DATA, "updates"),
    )
    phenopacket_schema_version = models.CharField(
        max_length=200, blank=True, help_text="Schema version of the current phenopacket."
    )
    external_references = JSONField(
        blank=True,
        null=True,
        validators=[JsonSchemaValidator(schema_ref=SchemaRefs.VALIDATION_PHENOPACKET_EXTERNAL_REFERENCE_SCHEMA)],
...
class PhenotypicFeature(BaseTimeStamp, IndexableMixin, ToFTSReprMixin):
    ...
    pftype = JSONField(
        verbose_name="type", validators=[ontology_validator], help_text=rec_help(d.PHENOTYPIC_FEATURE, "type")
    )
    excluded = models.BooleanField(default=False, help_text=rec_help(d.PHENOTYPIC_FEATURE, "excluded"))
    severity = JSONField(
        blank=True, null=True, validators=[ontology_validator], help_text=rec_help(d.PHENOTYPIC_FEATURE, "severity")
    )
    modifiers = JSONField(
        blank=True,
        null=True,
    ...
    onset = JSONField(
        blank=True, null=True, validators=[JsonSchemaValidator(schema_ref=SchemaRefs.TIME_ELEMENT_SCHEMA)]
    )
    ...
    evidence = JSONField(
        blank=True,
        null=True,
        validators=[JsonSchemaValidator(schema_ref=SchemaRefs.PHENOPACKET_EVIDENCE_SCHEMA)],
...
class Disease(BaseTimeStamp, IndexableMixin, ToFTSReprMixin):
    ...
    disease_stage = JSONField(
        blank=True, null=True, validators=[ontology_list_validator], help_text=rec_help(d.DISEASE, "disease_stage")
    )
    clinical_tnm_finding = JSONField(
        blank=True, null=True, validators=[ontology_list_validator], help_text=rec_help(d.DISEASE, "tnm_finding")
    )
...
class Biosample(BaseExtraProperties, BaseTimeStamp, BaseFTSModel, IndexableMixin, BaseScopeableModel):
    ...
    derived_from_id = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="derived_biosamples",
    ...
    sampled_tissue = JSONField(
        blank=True, null=True, validators=[ontology_validator], help_text=rec_help(d.BIOSAMPLE, "sampled_tissue")
    )
    sample_type = JSONField(
        blank=True, null=True, validators=[ontology_validator], help_text=rec_help(d.BIOSAMPLE, "sample_type")
    )
    ...
    taxonomy = JSONField(
        blank=True, null=True, validators=[ontology_validator], help_text=rec_help(d.BIOSAMPLE, "taxonomy")
    )
    time_of_collection = JSONField(
        blank=True,
        null=True,
    ...
    # TODO: Lists?
    tumor_progression = JSONField(
        blank=True, null=True, validators=[ontology_validator], help_text=rec_help(d.BIOSAMPLE, "tumor_progression")
    )
    tumor_grade = JSONField(
        blank=True, null=True, validators=[ontology_validator], help_text=rec_help(d.BIOSAMPLE, "tumor_grade")
    )
    pathological_stage = JSONField(
        blank=True, null=True, validators=[ontology_validator], help_text=rec_help(d.BIOSAMPLE, "pathological_stage")
    )
    pathological_tnm_finding = JSONField(
        blank=True,
        null=True,
        validators=[ontology_list_validator],
        help_text=rec_help(d.BIOSAMPLE, "pathological_tnm_finding"),
    )
    diagnostic_markers = JSONField(
        blank=True,
        null=True,
        validators=[ontology_list_validator],
    ...
    location_collected = models.ForeignKey(
        geo_models.GeoLocation,
        on_delete=models.CASCADE,
        blank=True,
        null=True,
        help_text=rec_help(d.BIOSAMPLE, "location_collected"),
    )
...
class VariationDescriptor(BaseTimeStamp, ToFTSReprMixin):
    ...
    variation = models.JSONField(
        blank=True,
        null=True,
        help_text=rec_help(d.VARIANT_DESCRIPTOR, "variation"),
    ...
    gene_context = models.ForeignKey(
        GeneDescriptor,
        blank=True,
        null=True,
        on_delete=models.CASCADE,
    ...
    expressions = models.JSONField(
        blank=True,
        null=True,
        validators=[JsonSchemaValidator(schema_ref=SchemaRefs.EXPRESSION_SCHEMA)],
        help_text=rec_help(d.VARIANT_DESCRIPTOR, "expressions"),
    )
    vcf_record = models.JSONField(
        blank=True, null=True, validators=[JsonSchemaValidator(schema_ref=SchemaRefs.VCF_RECORD_SCHEMA)]
    )
    xrefs = ArrayField(
        models.CharField(max_length=200, blank=True),
        blank=True,
    ...
    extensions = ArrayField(
        models.JSONField(
            blank=True, null=True, validators=[JsonSchemaValidator(schema_ref=SchemaRefs.EXTENSION_SCHEMA)]
        ),
        blank=True,
        default=list,
        help_text=rec_help(d.VARIANT_DESCRIPTOR, "extensions"),
    )
    molecule_context = models.CharField(
        max_length=200, blank=True, help_text=rec_help(d.VARIANT_DESCRIPTOR, "molecule_context")
    )
    structural_type = models.JSONField(
        blank=True,
        null=True,
        validators=[ontology_validator],
        help_text=rec_help(d.VARIANT_DESCRIPTOR, "structural_type"),
    )
    vrs_ref_allele_seq = models.CharField(
        max_length=200, blank=True, help_text=rec_help(d.VARIANT_DESCRIPTOR, "vrs_ref_allele_seq")
    )
    allelic_state = models.JSONField(
        blank=True,
        null=True,
        validators=[ontology_validator],
        help_text=rec_help(d.VARIANT_DESCRIPTOR, "allelic_state"),
    )
...
class VariantInterpretation(BaseTimeStamp, ToFTSReprMixin):
    ...
    acmg_pathogenicity_classification = models.CharField(
        max_length=200,
        choices=VARIANT_INTERPRETATION_STATUS,
        default="NOT_PROVIDED",
        help_text=rec_help(d.VARIANT_INTERPRETATION, "acmg_pathogenicity_classification"),
    )
    therapeutic_actionability = models.CharField(
        max_length=200,
        choices=THERAPEUTIC_ACTIONABILITY_CHOICES,
        default="UNKNOWN_ACTIONABILITY",
        help_text=rec_help(d.VARIANT_INTERPRETATION, "therapeutic_actionability"),
    )
    variation_descriptor = models.ForeignKey(
        VariationDescriptor, on_delete=models.CASCADE, help_text=rec_help(d.VARIANT_INTERPRETATION, "variant")
    )
...
class GenomicInterpretation(BaseTimeStamp, ToFTSReprMixin):
    ...
    interpretation_status = models.CharField(
        max_length=200,
        choices=GENOMIC_INTERPRETATION_STATUS,
    ...
    gene_descriptor = models.ForeignKey(
        GeneDescriptor,
        on_delete=models.CASCADE,
        null=True,
        blank=True,
    ...
    variant_interpretation = models.ForeignKey(
        VariantInterpretation,
        on_delete=models.CASCADE,
        null=True,
        blank=True,
        help_text="Corresponds to 'call' field in schema in case of VariantInterpretation",
    )
...
class Diagnosis(BaseTimeStamp, ToFTSReprMixin):
    ...
    id = models.CharField(
        primary_key=True, max_length=200, help_text="Unique ID, uses the parent Interpretation's id when ingesting."
    )
...
class Interpretation(BaseTimeStamp, ToFTSReprMixin):
    ...
    progress_status = models.CharField(
        choices=PROGRESS_STATUS, max_length=200, blank=True, help_text="The current status of work on the case."
    )
    diagnosis = models.OneToOneField(
        Diagnosis, blank=True, null=True, on_delete=models.CASCADE, help_text="The diagnosis, if made."
    )
...
class Phenopacket(BaseExtraProperties, BaseTimeStamp, BaseScopeableModel, BaseFTSModel, IndexableMixin):
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["id", "dataset_id"], name="unique_pheno_dataset")]
        indexes = [
            # JSON indices:
    ...
    subject = models.ForeignKey(
        Individual,
        blank=True,
        null=True,  # phenopackets DO NOT REQUIRE a subject!
        on_delete=models.CASCADE,  # if Individual instance is deleted Phenopacket instance is deleted too
