...
# Date patterns
#  - Date bin pattern: machine-readable "yyyy" or "yyyy-mm" bin key, as generated by labelled_date_range_generator
P_DATE_BIN = re.compile(r"^\d{4}(-\d{2})?$")
#  - Date range patterns
...
async def get_field_options(
    ...
    if field_props.datatype == "string":
        ...
        start, end = await get_date_range(queryset_entity, queryset, field_props, threshold)
...
async def get_range_stats(
    ...
    if isinstance(field_props, NumberFieldDefinition):
        ...
        threshold = get_threshold(scope, field_permissions)
        start, end = await get_date_range(queryset_entity, queryset, field_props, threshold)
        bin_ranges = tuple(f_utils.labelled_date_range_generator(field_props, start, end))
