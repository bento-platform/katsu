...
workflow experiments_json_with_files {
    ...
    call path_array_for_drs {
        input:
            json_path_list = prepare_files_list.path_list,
    }

    scatter (path in path_array_for_drs.consolidated_paths_for_drs) {
        call post_to_drs {
            input:
                file_path = path,
                drs_url = drs_url,
...
task prepare_files_list {
    # This task builds a JSON list of files collected from experiment results which exist in the directory we're pulling
    # files from, optionally skipping VCFs.
    # The resulting path_list output is of the format [{"filename": "...", "path": "..."}, ...]
...
task path_array_for_drs {
    # This task, given a JSON list of file entries of the form generated by prepare_files_list, produces a WDL array of
    # just the paths (`consolidated_paths_for_drs`).
    # So, we go from JSON [{"filename": "...", "path": "..."}, ...] --> WDL ["path1", "path2", etc.]
