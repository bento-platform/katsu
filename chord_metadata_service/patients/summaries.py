...
async def individual_summary(phenopackets: QuerySet | None, discovery: DiscoveryConfig, low_counts_censored: bool):
    individuals = (
    ...
    individual_count, individual_sex, individual_k_sex, individual_age, individual_taxonomy = await asyncio.gather(
        ...
        queryset_stats_for_field(individuals, "sex", discovery, low_counts_censored),
        queryset_stats_for_field(individuals, "karyotypic_sex", discovery, low_counts_censored),
        #  - Age
        get_age_numeric_binned(individuals, OVERVIEW_AGE_BIN_SIZE, discovery, low_counts_censored),
        #  - Taxonomy
        queryset_stats_for_field(individuals, "taxonomy__label", discovery, low_counts_censored),
    )

    return {
        "count": thresholded_count(individual_count, discovery, low_counts_censored),
