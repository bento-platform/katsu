...
class FHIRRenderer(JSONRenderer):
    ...
    def render(self, data, media_type=None, renderer_context=None):
        ...
        if "results" in data:
            final_data = {
                fhir_datatype_plural: [
                    class_converter(item) for item in data["results"]
                ]
            }
        else:
            final_data = class_converter(data)
        return super(FHIRRenderer, self).render(
            final_data, media_type, renderer_context
        )
...
class PhenopacketsRenderer(CamelCaseJSONRenderer):
    ...
    def render(self, data, media_type=None, renderer_context=None):
        return super(PhenopacketsRenderer, self).render(
...
class JSONLDDatasetRenderer(PhenopacketsRenderer):
    ...
    def render(self, data, media_type=None, renderer_context=None):
        if "results" in data:
            json_obj = {
                "results": [dataset_to_jsonld(item) for item in data["results"]]
            }
        else:
            json_obj = dataset_to_jsonld(data)

        return super(JSONLDDatasetRenderer, self).render(
            json_obj, media_type, renderer_context
        )
    ...
    def render(self, data, media_type=None, renderer_context=None):
        if "results" in data:
            ...
            for item in data["results"]:
                ...
                small_g = Graph().parse(
                    data=json.dumps(ld_context_item, cls=UUIDEncoder), format="json-ld"
                )
                # join graphs
                g = g + small_g
        else:
            ...
            g = Graph().parse(
                data=json.dumps(ld_context_data, cls=UUIDEncoder), format="json-ld"
            )
...
class IndividualCSVRenderer(JSONRenderer):
    ...
    def render(self, data, media_type=None, renderer_context=None):
        ...
        headers = {
            key: key.replace("_", " ").capitalize() for key in individuals[0].keys()
        }
